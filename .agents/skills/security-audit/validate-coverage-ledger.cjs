...
function readFileWithinLimit(file) {
  ...
  const noFollow = isWindows ? 0 : fs.constants.O_NOFOLLOW;
  const nonBlock = isWindows ? 0 : fs.constants.O_NONBLOCK;
  ...
  try {
    if (isWindows) {
      ...
      if (lstat.isSymbolicLink()) throw new SafeInputError("input must not be a symlink");
